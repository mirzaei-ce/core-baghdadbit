...
###   This script attempts to download the signature file SHA256SUMS.asc from baghdadbit.org
...
WORKINGDIR="/tmp/baghdadbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://baghdadbit.org/bin/"
VERSIONPREFIX="baghdadbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'baghdadbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Baghdadbit Core binary release signing key installed?"
   fi
